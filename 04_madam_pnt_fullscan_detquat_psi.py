...
if __name__ == "__main__":
    ...
    for i_det, det in enumerate(dets):
        loc_det_v = qa.rotate(np.array(fp[det]["quat"]), z_axis)[0]
        loc_det_v_x = qa.rotate(qa.rotation(y_axis, np.pi / 2), loc_det_v)
        det_v = qa.rotate(bore_quat, loc_det_v_x)

        pixels[i_det * nsamp : (i_det + 1) * nsamp] = hp.vec2pix(
            nside, det_v[:, 0], det_v[:, 1], det_v[:, 2], nest=True
        )

        # polarization weights

        det_v_proj_ortog = np.hstack(
            [-det_v[:, [1]], det_v[:, [0]], np.zeros(len(det_v))[:, np.newaxis]]
        )
        det_v_proj_ortog /= np.linalg.norm(det_v_proj_ortog, axis=1)[:, np.newaxis]
        ...
        psi[det] = np.arccos(qa.arraylist_dot(local_north_v_fp, loc_pol_v_z).flatten())
